...
sklm: {
    ...
    remoteHosts: {
      ...
      pairTitle: "配對新主機",
      pairBody: "如果你已自行執行 pi-host：貼上它的 RACP 位址與啟動輸出中的配對權杖。",
      pair: "配對",
      pairing: "配對中…",
      pairAction: "新增",
      ...
      fieldUrl: "RACP 位址",
      fieldUrlDesc:
        "pi-host 的迴環端點，例如 ws://127.0.0.1:9443/v1/racp/ws（經 SSH 轉發）。適用於你自己啟動的主機；下方的 SSH 安裝會替你完成這一步。",
      fieldPairingToken: "配對權杖",
      fieldPairingTokenDesc:
        "來自 pi-host 引導輸出的一次性 ppt1.* 權杖，配對後即消耗。",
      sshTitle: "在伺服器上安裝（SSH）",
      sshBody:
        "PI-Desktop 會在你已能用 ssh 登入的機器上安裝 pi-host，並保持一條迴環通道。認證沿用你自己的 ssh 設定與 agent；也可以用登入密碼代替金鑰。",
      sshHost: "SSH 主機",
      sshHostDesc: "主機名稱或位址，與 ssh 收到的原樣相同。",
      sshHostPlaceholder: "build-box.local",
      sshUser: "使用者",
      sshUserDesc: "登入使用者；留空則使用本機使用者名稱。",
      sshPort: "SSH 連接埠",
      sshPortDesc: "留空使用 ssh 預設值（22）。",
      sshIdentityFile: "私密金鑰檔案",
      sshIdentityFileDesc: "私密金鑰路徑；留空則由 agent 與 ~/.ssh/config 決定。",
      sshAuthMode: "認證方式",
      sshAuthModeDesc: "使用你自己的 SSH 金鑰或 agent，也可以改用密碼認證。",
      sshAuthKey: "SSH 金鑰",
      sshAuthPassword: "密碼",
      sshPassword: "密碼",
      sshPasswordDesc: "透過 askpass 輔助程式交給 ssh 用戶端，不會出現在命令列上。",
      sshPasswordPlaceholder: "你的登入密碼",
      sshShowPassword: "顯示密碼",
      sshHidePassword: "隱藏密碼",
      sshPasswordStored: "儲存在系統鑰匙圈中，主機在重新啟動後也能重新連線。",
      sshAction: "安裝並配對",
      sshActionHint: "在目標主機上安裝 pi-host，並與本桌面配對。",
      sshRunning: "安裝中…",
      sshSucceeded: "已安裝並配對 {{label}}。",
