...
#### E2E-UI-topbar-session-context: Branch and open-location controls
...
- **Steps**: 1) Click the branch badge and paste elsewhere. 2) Click the
  open-location main button and confirm the directory opens in the preferred
  app. 3) Open the caret menu, choose a different app, and confirm the
  directory opens there. 4) Click the main button again and confirm the newly
  chosen app is now the default. 5) Open a projectless temporary chat and
  confirm the cluster hides or falls back to the workspace root. 6) Open a
  conversation in a non-git directory and confirm the branch badge hides while
  open-location remains.
- **Expected**: The branch badge copies the branch name and shows brief check
