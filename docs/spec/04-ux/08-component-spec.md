...
> 布局和 IA 参考：[01-ui-ia.md](/spec/04-ux/01-ui-ia)
> 设计代币和基础：[07-ui-design-system.md](/spec/04-ux/07-ui-design-system)
> 交互行为：[09-interaction-patterns.md](/spec/04-ux/09-interaction-patterns)

> Shell 布局与 Codex 对齐：左线程侧边栏（固定 275px）、主记录、带有运行时 mode/permission/model 控件的浮动底部编辑器，以及仅包含操作按钮的紧凑顶栏。与蓝板岩镀铬相比，更喜欢中性木炭表面。
>
> **优先规则**：下面的指标或复制字符串与
> [决策日志 §D](/spec/08-meta/decisions-log) 中的法典平价决策
...
- 窗口大小调整：本机边缘仅在打开作品时更改 MainChat 宽度
  面板保持其承诺的宽度；响应式布局如下
  [07-ui-design-system.md](/spec/04-ux/07-ui-design-system) §10.1
...
切换**仅在侧边栏折叠时**；当侧边栏展开时由侧边栏拥有该控件，
因此顶部栏不再重复它。新建任务与全局搜索入口位于侧边栏（§3）：
标题下方的主新建任务行和标题操作区的搜索图标。侧边栏折叠期间，
顶栏前部插槽会镜像这些入口（侧边栏开关、新建任务、搜索）。键盘快捷键
和应用菜单仍然可用。）
...
- 顶部栏中没有搜索字段，也没有新建/搜索操作按钮；两者均位于侧边栏（§3）。
...
- 单击标题行右侧的“折叠”侧栏以折叠侧边栏。
  全局搜索从侧边栏标题操作区的搜索图标、快捷键和应用菜单打开；标题下方的主新建任务行与应用菜单的“新建任务”命令一致：在当前范围创建会话并聚焦输入框
...
[03-permission-ux.md](/spec/04-ux/03-permission-ux) 了解完整策略。
...
- 原始会话的输入框无法在等待许可期间发送，
  同时文本保持可编辑（根据 [03-permission-ux.md](/spec/04-ux/03-permission-ux) §7）
- 中止取消待处理的权限
...
- 仅限内联卡；没有模态或背景后备
- 没有“始终允许”选项（根据 [03-permission-ux.md](/spec/04-ux/03-permission-ux)）
- 无风险级别定制
...
| 正在等待许可 | 文本区域已禁用（根据 [03-permission-ux.md](/spec/04-ux/03-permission-ux) §7） | 发送禁用；满足运行中且草稿为空时停止仍然可用 |
...
- 没有动画空状态插图
- 无产品介绍叠加（根据 [05-onboarding.md](/spec/04-ux/05-onboarding) §6）
...
**状态：合并到全局搜索界面中。**命令面板覆盖已删除；其命令列表（内置+插件命令）现在呈现为 `SearchDialog` 内的“命令”部分（使用 Cmd/Ctrl+K 或 Cmd/Ctrl+Shift+P 打开）。在 [04-builtin-commands.md](/spec/04-ux/04-builtin-commands) 中定义，并由全局搜索组件规范显示。
...
- Surface：`bg-elevated-opaque` + 1px `border-subtle` + `shadow-dialog`，radius-md-plus — 与菜单相同的浮动系列； [07-ui-design-system.md §11.8](/spec/04-ux/07-ui-design-system#_11-8-toast) 中的指标
...
1. 所有组件均使用 [07-ui-design-system.md](/spec/04-ux/07-ui-design-system) 中的语义颜色标记 — 无原始十六进制
2.所有交互元素都有可见的聚焦环（2px强调，偏移2px）
3. 布局 shell 指标（46px 标题栏行、~275/48 侧边栏、280 上下文、
具有 1-7 行草稿增长的紧凑型输入框）匹配规范
4. 聊天消息最大宽度限制为 720px
5. ToolCallCard 根据 [01-ui-ia.md](/spec/04-ux/01-ui-ia) §5 显示状态、参数预览、结果预览、持续时间
6. PermissionCard 显示工具名称、风险、参数、倒计时和每个 [03-permission-ux.md](/spec/04-ux/03-permission-ux) 的三个操作按钮
