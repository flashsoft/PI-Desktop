...
> M5 硬化。交叉引用：[日志记录](/spec/03-runtime/09-logging-and-observability)
> · [进程模型](/spec/03-runtime/07-process-model) · [插件安全](/spec/07-plugins/04-plugin-security)
...
[远程控制安全](/spec/05-security/02-remote-control-security) 和
[远程 Agent Control 架构](/spec/02-architecture/05-remote-agent-control)。
本地安全基线不会因为远程目标规格而开放新的监听器或权限通道。

## 3. 秘密

- 通过 Electron `safeStorage` 加密存储的密钥，由 host-core 管理
  （参见 [14 个密钥存储](/spec/03-runtime/14-secrets-storage)）
...
- 超时是强制性的：默认为 60 秒，可覆盖 1–21,600 秒（D329）。输出
  ...
  （见 [16-tool-result-limits](/spec/03-runtime/16-tool-result-limits)）
- 用户中止和超时在工具之前关闭完整的进程树
  关闭；没有孤儿进程可以继续写入输出。
- 审计日志中记录的完整命令行（SQLite，已编辑），带有 shell ID
  和方言而不是不受信任的可执行路径或路径哈希
- Allowlist/denylist细化是有跟踪的后续
  ([03-工具和权限](/spec/03-runtime/03-tools-and-permissions))
...
- 本地化产品“新增内容”文本 (D164/D345) 在 Main 中从
  ...
  条目只是省略了该部分。
- 开发者 ID + 公证通道仍记录在
  [发布运行手册](/spec/06-delivery/06-release-runbook)。
