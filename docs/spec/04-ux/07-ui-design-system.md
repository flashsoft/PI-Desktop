...
2. 完整的组件库规范（即[08-component-spec.md](/spec/04-ux/08-component-spec)）
...
> 另请参阅 [09-interaction-patterns.md](/spec/04-ux/09-interaction-patterns) §10。
...
这些指标定义了 AppShell 框架。有关组件详细信息，请参阅 [08-component-spec.md](/spec/04-ux/08-component-spec)。
...
这些是通用原语的**令牌级基础**。详细的组件规格位于 [08-component-spec.md](/spec/04-ux/08-component-spec)。
...
完整的组件合同和使用规则：[08-component-spec.md §17](/spec/04-ux/08-component-spec#_17-toast)。
...
- 默认折叠长内容（工具结果、长消息）— 请参阅 [09-interaction-patterns.md](/spec/04-ux/09-interaction-patterns) §4
...
- **项目档案**：D066 Codex 索引表（搜索/展开/操作）
  ...
  完全一致：一条仅含说明的安静引导行；
  一个工具栏（排序分段控件、搜索、主要操作）；以及一个索引，
  其固定/所有项目/已存档分组是纯标签式的分组标题行，并承载页面上唯一的计数。
  根据 D455 它保持单列、没有并排面板，且该索引是 iOS 意义上的内嵌分组列表：
  每行自左向右是身份（字形、项目名、状态标签、路径），自右向左是细节
  （会话计数、上次活动、展开指示器），选中的行就是在其下方展开的卡片的表头 ——
  因此详情不会重复名称、路径或标签。它没有英雄区块、装饰渐变，也没有页面级计数器串
- **设置**：按照 D063/D090/D133/D166 的全页 Codex shell（275 像素紧凑型
  ...
  200px 轨道和广泛的分组目录被取代
- **导入**：四种类型（会话 / 模型 / 技能 / MCP）位于一个页面级分段切换器之后，
  其构成与智能体能力页面一致：每个类型一个安静的扫描前下一步操作状态、
  每个类型一个工具栏（全选连同两个计数、该类型自己的选项、重新扫描、导入所选），
  以及一个列表，其分组标题是安静的标签行、候选项是独立磁贴。没有按类型划分的
  扫描卡片、没有着色的分组条带，也没有第二份设置行脚手架副本
